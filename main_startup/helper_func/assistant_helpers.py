...
def _check_admin(func):
    ...
    async def magic_admin(client, message):
        ...
        if is_a_o:
            ...
            await message.reply_text("`>> Bunu Yapabilmek İçin Yönetici / Sahip Olmalısınız! >>`")
...
def _check_owner_or_sudos(func):
    ...
    async def magic_owner(client, message):
        ...
        if message.from_user.id in use_:
            ...
            await message.reply_text("`>> Bunu Yapmak İçin Sahip veya Sudo olmalısın! >>`")
...
async def download_yt(url, as_video=False):
    ...
    try:
        ...
        return f"**İndirilemedi** \n**Hata :** `{str(e)}`", None, None, None
    yt_id = ytdl_data['id']
